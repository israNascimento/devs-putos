...
const WORDS = [
	...
	"buceta",
	"caralho",
	"vai tomar no cu", "vtnc",
	"desgraca",
	"foda", "foda-se", "fodido", "fodase",
	"merda",
	"porra", 
	"puta"
];
...
function searchOlder() {
	...
	console.log('Buscando a palavra: '+githubParams.q+" na página: "+githubParams.page);
	fetchOnGit(githubParams)
		.then((response) => {
...
function postToTwitter(item) {
	...
	TwitterAPI.post("statuses/update", { status: message }).then(value => {
...
function getMessage(item) {
	...
	return "O/A dev: "+author+"\ncommitou: \""+commit.message+"\"\n\nlink do repositório: "+getRepositoryURL(item);
}
